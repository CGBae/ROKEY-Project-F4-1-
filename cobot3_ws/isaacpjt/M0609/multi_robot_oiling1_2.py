...
from geometry_msgs.msg import PoseStamped, Vector3Stamped  # 위치+방향/벡터 메시지 타입 (카메라가 찾은 좌표와 삽입 방향을 받을 때 사용)
...
# ╚══════════════════════════════════════════════════════════════╝
PHYSICS_DT = 2.0 / 60.0       # 물리 시뮬레이션 한 스텝의 시간(초). 60Hz -> 약 16.7ms마다 한 번
...
RETREAT_TARGET_SPEED = 0.100   # 빼거나 후퇴할 때
...
COVER_CLEARANCE_DISTANCE = 0.60  # 마개로 가기 전에 들르는 경유점의 벽 바깥쪽 거리(m) - 열린 커버를 피해서 돌아가기 위함
...
DOOR_AUTO_OPEN_DRIVE_VELOCITY = 15.0  # USD에 이미 설정된 덮개 자동 열기 velocity 드라이브의 targetVelocity 값
...
DOOR_CLOSE_OUTWARD_CLEARANCE = 0.25
...
CAP_JOINT6_DEG_PER_STEP = 8.0     # 한 스텝마다 최대 몇 도씩 회전시킬지 (너무 빨리 돌면 부자연스러움)
...
# ArUco detector가 발행하는 /color_detector/direction은 주유구 안쪽으로 들어가는 삽입 방향이다.
# RMPFlow orientation 목표를 만들 때, 로봇 link_6의 어느 local 축을 이 삽입 방향에 맞출지 정한다.
# 노즐 USD가 link_6의 다른 축을 따라 붙어 있다면 이 값만 바꿔서 맞춘다.
EE_TOOL_FORWARD_AXIS_LOCAL = np.array([0.0, 0.0, 1.0], dtype=float)
REQUIRE_TARGET_DIRECTION = True
DIRECTION_WORLD_STD_TOLERANCE = 0.18  # 방향 샘플 흔들림 허용치. 단위벡터 표준편차 norm 기준
...
WAIT_LOCK_TIMEOUT_STEPS = 600                # 이 스텝 수가 지나도 A 주유구 lock이 안 되면 하드코딩 폴백 없이 샘플을 비우고 재수집 (~10초 @ 60fps)

WAIT_DOOR_OPEN_HOLD_STEPS = 20
WAIT_DOOR_OPEN_TIMEOUT_STEPS = 360  # 이 스텝 수가 지나도 B가 덮개를 열지 못하면 강제로 다음 단계로 넘어감 (~10초 @ 60fps)

# ╔══════════════════════════════════════════════════════════════╗
# ║  E. ROS2 토픽 이름                                              ║
# ╚══════════════════════════════════════════════════════════════╝
TOPIC_COLOR_POSE = "/color_detector/pose"          # 카메라가 찾은 ArUco 기준 목표점의 3D 위치
TOPIC_COLOR_DIRECTION = "/color_detector/direction"  # ArUco 기준 주유구 삽입 방향 벡터(Vector3Stamped, camera frame)
...
def transform_camera_vector_to_world(vector_camera_ros: np.ndarray, camera_prim_path: str | None = None) -> np.ndarray:
    """카메라 optical frame의 방향 벡터를 world 방향 벡터로 바꾼다.

    위치 변환과 같은 캘리브레이션 매핑을 쓰되, 방향 벡터이므로 카메라 원점 translation은 더하지 않는다.
    현재 씬의 카메라 축 매핑은 (x, y, z)_camera -> (-x, -z, -y)_world 로 보정되어 있다.
    """
    x_cam = float(vector_camera_ros[0])
    y_cam = float(vector_camera_ros[1])
    z_cam = float(vector_camera_ros[2])
    return normalize(np.array([-x_cam, -z_cam, -y_cam], dtype=float))


def quat_wxyz_normalize(q: np.ndarray, eps: float = 1e-9) -> np.ndarray:
...
def quat_wxyz_to_rot_matrix(q: np.ndarray) -> np.ndarray:
    """Isaac Sim/RMPFlow에서 쓰는 wxyz quaternion을 3x3 회전행렬로 바꾼다."""
...
def quat_wxyz_multiply(q_left: np.ndarray, q_right: np.ndarray) -> np.ndarray:
    """wxyz quaternion 곱. q_left 회전 뒤 q_right가 아니라, 행렬 기준 R = R_left @ R_right."""
    w1, x1, y1, z1 = quat_wxyz_normalize(q_left)
    w2, x2, y2, z2 = quat_wxyz_normalize(q_right)
    return quat_wxyz_normalize(np.array([
...
def quat_wxyz_from_two_vectors(v_from: np.ndarray, v_to: np.ndarray, eps: float = 1e-9) -> np.ndarray:
    """v_from 방향을 v_to 방향으로 돌리는 최소 회전 quaternion(wxyz)을 만든다."""
    a = normalize(np.array(v_from, dtype=float), eps)
    b = normalize(np.array(v_to, dtype=float), eps)
    ...
    if dot > 1.0 - 1e-6:
        return np.array([1.0, 0.0, 0.0, 0.0], dtype=float)
    if dot < -1.0 + 1e-6:
        # 180도 반대 방향이면 a와 수직인 임의 축을 골라 회전한다.
        aux = np.array([1.0, 0.0, 0.0], dtype=float)
        if abs(a[0]) > 0.9:
            aux = np.array([0.0, 1.0, 0.0], dtype=float)
        axis = normalize(np.cross(a, aux), eps)
        return np.array([0.0, axis[0], axis[1], axis[2]], dtype=float)

    axis = np.cross(a, b)
    q = np.array([1.0 + dot, axis[0], axis[1], axis[2]], dtype=float)
    return quat_wxyz_normalize(q)


def make_orientation_aligning_local_axis(current_quat_wxyz: np.ndarray,
                                         local_axis: np.ndarray,
                                         target_world_axis: np.ndarray) -> np.ndarray:
    """현재 EE 자세에서 local_axis가 target_world_axis를 향하도록 기울인 목표 orientation을 만든다.

    예: EE_TOOL_FORWARD_AXIS_LOCAL=[0,0,1]이면 link_6 local +Z축이 ArUco 삽입 방향을 보게 된다.
    """
    current_q = quat_wxyz_normalize(np.array(current_quat_wxyz, dtype=float))
    target_axis = normalize(np.array(target_world_axis, dtype=float))
    if np.linalg.norm(target_axis) < 1e-9:
        return current_q
    local_axis = normalize(np.array(local_axis, dtype=float))
    if np.linalg.norm(local_axis) < 1e-9:
        return current_q

    current_R = quat_wxyz_to_rot_matrix(current_q)
    current_axis_world = normalize(current_R @ local_axis)
    delta_q = quat_wxyz_from_two_vectors(current_axis_world, target_axis)
    return quat_wxyz_multiply(delta_q, current_q)
...
# ============================================================
def build_fuel_port_sequence(fuel_port_center: np.ndarray, insertion_direction_world: np.ndarray) -> WaypointSequence:
    """A 로봇이 ArUco가 발행한 위치+삽입 방향 벡터를 기준으로 주유구에 접근/삽입/후퇴하는 시퀀스.

    fuel_port_center: /color_detector/pose를 world로 변환하고 안정화한 주유구 중심 좌표
    insertion_direction_world: /color_detector/direction을 world로 변환한 삽입 방향 단위벡터
    """
    insertion = normalize(np.array(insertion_direction_world, dtype=float))
    if np.linalg.norm(insertion) < 1e-9:
        raise ValueError("A 로봇 주유 시퀀스를 만들 수 없음: insertion_direction_world가 0벡터입니다.")
    outward = -insertion

    # 노즐 tip이 지나갈 점들. 이제 하드코딩 PORT_OUTWARD_NORMAL이 아니라 ArUco 방향 벡터를 기준으로 계산한다.
    ...
    # 로봇이 실제로 제어하는 점은 노즐 tip이 아니라 link_6 손목이므로,
    # tip에서 노즐 길이만큼 바깥쪽으로 뺀 지점을 link_6 목표로 사용한다.
    ...
    stages = [
        # 처음 접근은 위치 위주로 이동한다.
        FuelStage("01_vector_far_start", approach_far, tolerance=0.08, speed=DEFAULT_TARGET_SPEED, use_orientation=False),
        FuelStage("02_vector_mid", approach_mid, tolerance=0.07, speed=DEFAULT_TARGET_SPEED, use_orientation=False),
        FuelStage("03_vector_near_pre_align", approach_near, hold_steps=20, tolerance=0.06,
                  speed=NEAR_TARGET_SPEED, use_orientation=False),

        # 직선 삽입 전에 같은 near 위치에서 ArUco 방향 벡터 기준으로 손목/노즐 방향을 먼저 기울인다.
        FuelStage("04_tilt_to_aruco_vector", approach_near, hold_steps=70, tolerance=0.06,
                  speed=NEAR_TARGET_SPEED, use_orientation=True),

        # 방향 정렬 후에는 같은 삽입축을 따라 직선으로 들어간다.
        FuelStage("05_insert_along_aruco_vector", insert_target, hold_steps=80, tolerance=0.05,
                  speed=INSERT_TARGET_SPEED, use_orientation=True),
        FuelStage("06_retreat_near", approach_near, tolerance=0.08, speed=RETREAT_TARGET_SPEED, use_orientation=True),
        FuelStage("07_retreat_mid", approach_mid, tolerance=0.08, speed=RETREAT_TARGET_SPEED, use_orientation=True),
        FuelStage("08_retreat_far", approach_far, hold_steps=15, tolerance=0.08,
                  speed=RETREAT_TARGET_SPEED, use_orientation=True),
        FuelStage("09_return_home", None),
    ]
    stage_log = {
        "01_vector_far_start": "A ArUco 방향 기준 접근 시작",
        "02_vector_mid": "A ArUco 방향 기준 중간 지점",
        "03_vector_near_pre_align": "A 삽입 전 근접 대기",
        "04_tilt_to_aruco_vector": "A 삽입 방향 벡터로 손목 기울이기",
        "05_insert_along_aruco_vector": "A ArUco 삽입축 직선 삽입 완료",
        "06_retreat_near": "A 삽입축 후퇴 시작",
        "07_retreat_mid": "A 중간 후퇴",
        "08_retreat_far": "A 후퇴 완료",
    }
    print(f"[A][SEQ] ArUco 기반 주유 경로 생성 center={np.round(fuel_port_center, 4)} "
          f"insert_dir={np.round(insertion, 4)} outward={np.round(outward, 4)}")
...
class MultiRobotRosBridge(Node):
    ...
    def __init__(self):
        ...
        self.latest_direction: "Vector3Stamped | None" = None  # 가장 최근에 받은 ArUco 삽입 방향 벡터
        self.pose_count = 0       # pose가 몇 번 도착했는지 누적 카운트 (새 메시지 도착 여부 판단용)
        self.direction_count = 0  # direction이 몇 번 도착했는지 누적 카운트
        ...
        self.direction_sub = self.create_subscription(Vector3Stamped, TOPIC_COLOR_DIRECTION, self._direction_cb, sensor_qos)
    ...
    def _direction_cb(self, msg: Vector3Stamped):
        """ArUco detector가 발행한 주유구 삽입 방향 벡터를 저장한다."""
        self.latest_direction = msg
    ...
    def get_direction_if_ready(self):
        """direction 토픽도 lock=True일 때만 사용한다. pose와 함께 최신값만 사용한다."""
...
class StableTargetLockAcquirer:
    """노란/파란/초록 모드 공통: N개 샘플이 표준편차 이내로 모이면 평균 world 좌표를 반환한다.

    require_direction=True이면 /color_detector/direction도 함께 받아서 world 삽입 방향을 안정화한다.
    """
    ...
    def __init__(self, ros_bridge: MultiRobotRosBridge, camera_prim_path: "str | None",
                 ...
                 direction_std_tolerance: float = DIRECTION_WORLD_STD_TOLERANCE):
        ...
        self.std_tolerance = std_tolerance              # 위치 샘플들의 흔들림 허용치
        self.gate_half_extent = gate_half_extent        # 게이트(허용 범위) 크기
        self.require_direction = bool(require_direction)
        self.direction_std_tolerance = float(direction_std_tolerance)

        self.samples: list = []                         # 지금까지 모은 유효한 후보 위치들
        self.direction_samples: list = []               # 지금까지 모은 유효한 삽입 방향들(world 단위벡터)
        self.locked_direction_world: "np.ndarray | None" = None
        self.last_sampled_pose_count = -1               # 마지막으로 처리한 pose_count (중복 처리 방지)
        self.direction_count_at_start = ros_bridge.direction_count

    def reset(self):
        """모아둔 샘플을 전부 버리고 처음부터 다시 모으기 시작한다 (모드 전환/타임아웃 시 호출)."""
        self.samples = []
        self.direction_samples = []
        self.locked_direction_world = None
        self.last_sampled_pose_count = -1
        self.direction_count_at_start = self.ros_bridge.direction_count

    def update(self):
        """매 tick 호출. 안정화된 평균 world 좌표가 나오면 반환, 아니면 None.

        require_direction=True인 경우에는 위치와 방향이 모두 안정화되어야 반환한다.
        """
        ...
        direction_msg = self.ros_bridge.get_direction_if_ready()
        if direction_msg is not None and self.ros_bridge.direction_count > self.direction_count_at_start:
            d_cam = np.array([
                direction_msg.vector.x,
                direction_msg.vector.y,
                direction_msg.vector.z,
            ], dtype=float)
            direction_world = transform_camera_vector_to_world(d_cam, self.camera_prim_path)
            if np.linalg.norm(direction_world) < 1e-9:
                direction_world = None

        if self.require_direction and direction_world is None:
            # 위치는 들어왔지만 방향 토픽이 아직 green 모드 기준으로 갱신되지 않았으면 샘플로 쓰지 않는다.
            return None
        ...
        if self.apply_mouth_offset:
            if direction_world is not None:
                # ArUco가 준 삽입 방향 기준으로 입구 표면 -> 구멍 중심 보정
                candidate = detected_world_point + direction_world * (FUEL_PORT_DEPTH / 2.0)
            else:
                # 방향 토픽을 요구하지 않는 B/legacy 경로용 이전 보정
                candidate = detected_world_point_to_mouth_center(detected_world_point)
        else:
            candidate = detected_world_point
        ...
        if valid:
            ...
            if direction_world is not None:
                self.direction_samples.append(direction_world)
                self.direction_samples = self.direction_samples[-self.required_samples:]
        else:
            self.samples = []
            self.direction_samples = []
            self.locked_direction_world = None
        ...
        if len(self.samples) >= self.required_samples:
            ...
            if std_norm <= self.std_tolerance:
                if self.require_direction:
                    if len(self.direction_samples) < self.required_samples:
                        return None
                    dir_arr = np.array(self.direction_samples, dtype=float)
                    mean_dir = normalize(dir_arr.mean(axis=0))
                    dir_std_norm = float(np.linalg.norm(dir_arr.std(axis=0)))
                    if np.linalg.norm(mean_dir) < 1e-9 or dir_std_norm > self.direction_std_tolerance:
                        return None
                    self.locked_direction_world = mean_dir
                return mean
        return None
...
class RobotARunner:
    ...
    def __init__(self, robot, controller, ros_bridge: MultiRobotRosBridge, camera_prim_path,
        ...
        self.sequence = None
        self.locked_target_orientation = None  # ArUco 방향 벡터에 맞춘 손목 목표 자세
        self.locked_insert_direction_world = None
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "IDLE_WAIT_B":
            if self._robot_b_done_received():
                ...
                self.lock_acquirer = StableTargetLockAcquirer(
                    self.ros_bridge, self.camera_prim_path, self.task.hole_world_position,
                    apply_mouth_offset=True, require_direction=REQUIRE_TARGET_DIRECTION,
                )
                self.wait_steps = 0
        ...
        if self.run_state == "WAIT_LOCK_GREEN":
            ...
            if mean is not None:
                direction_world = self.lock_acquirer.locked_direction_world
                if direction_world is None or np.linalg.norm(direction_world) < 1e-9:
                    print("[A][WAIT_LOCK_GREEN] 위치는 lock 되었지만 direction이 아직 안정화되지 않음")
                    return

                _, current_ee_ori = self.robot.end_effector.get_world_pose()
                self.locked_insert_direction_world = normalize(direction_world)
                self.locked_target_orientation = make_orientation_aligning_local_axis(
                    current_ee_ori,
                    EE_TOOL_FORWARD_AXIS_LOCAL,
                    self.locked_insert_direction_world,
                )
                # A가 오래 대기하다가 처음 움직이므로 RMPFlow 내부 상태를 한 번 초기화
                try:
                    self.controller.reset()
                    print("[A][DEBUG] controller.reset() 완료")
                except Exception as e:
                    print(f"[A][WARN] controller.reset() 실패: {e}")

                self.sequence = build_fuel_port_sequence(mean, self.locked_insert_direction_world)

                # 생성된 A 목표점들을 한 번에 출력해서 target이 말이 되는지 확인
                print("\n[A][SEQ_TARGETS]")
                for i, st in enumerate(self.sequence.stages):
                    print(f"  {i:02d} {st.name}: target={None if st.target_position is None else np.round(st.target_position, 4)} "
                        f"use_ori={st.use_orientation} speed={st.speed}")

                self.run_state = "RUN_SEQUENCE"
                print(f"\n[A] green lock 완료. fuel_port target={np.round(mean, 4)} "
                    f"insert_dir_world={np.round(self.locked_insert_direction_world, 4)}\n")
                print(f"[A] target_orientation(wxyz)={np.round(self.locked_target_orientation, 4)}\n")
                return
            if self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
                # 더 이상 하드코딩 위치로 폴백하지 않는다. ArUco pose+direction이 안정화될 때까지 계속 대기한다.
                print("\n[A] WAIT_LOCK_GREEN timeout -> 하드코딩 경로 사용 안 함. ArUco pose/direction 재수집 계속 진행\n")
                self.wait_steps = 0
            ...
            if step_count % PRINT_EVERY_N_STEPS == 0:
                print(f"[A][WAIT_LOCK_GREEN] locked={self.ros_bridge.target_locked} "
                      f"pose_count={self.ros_bridge.pose_count} direction_count={self.ros_bridge.direction_count} "
                      f"lock_count={self.ros_bridge.lock_count} "
                      f"samples={len(self.lock_acquirer.samples)}/{CONTROLLER_REQUIRED_LOCK_SAMPLES} "
                      f"dir_samples={len(self.lock_acquirer.direction_samples)}/{CONTROLLER_REQUIRED_LOCK_SAMPLES}")
            return
        ...
        if self.run_state == "RUN_SEQUENCE":
            ...
            # 실제 ArUco 기반 주유 시퀀스 진행. 마지막 return_home 단계는 위치 목표가 없는
            ...
            if stage.target_position is None:
                ...
                if step_count % PRINT_EVERY_N_STEPS == 0:
                    print(f"[A][09_return_home] hold={self.sequence.hold_count}/{HOME_HOLD_STEPS}")
            ...
            if cmd is not None:
                before_joints = self.robot.get_joint_positions().copy()
                ...
                try:
                    ...
                    if actions is None:
                        print("[A][ERROR] controller.forward() returned None")
                        return
                    ...
                    if step_count % PRINT_EVERY_N_STEPS == 0:
                        ...
                        print(f"[A][ACTION] stage={stage.name} "
                            f"ee={np.round(ee_pos, 4)} "
                            f"cmd={np.round(cmd, 4)} "
                            f"target={np.round(stage.target_position, 4)} "
                            f"cmd_err={np.linalg.norm(cmd - ee_pos):.4f} "
                            f"final_err={np.linalg.norm(stage.target_position - ee_pos):.4f} "
                            f"use_ori={stage.use_orientation and USE_TARGET_ORIENTATION}")

                        if jp is None:
                            print("[A][ACTION] joint_positions=None")
                        else:
                            jp = np.array(jp, dtype=float)
                            print(f"[A][ACTION] target_joints[:6](deg)="
                                f"{np.round(np.degrees(jp[:6]), 2)}")
                            print(f"[A][ACTION] current_joints[:6](deg)="
                                f"{np.round(np.degrees(before_joints[:6]), 2)}")

                except Exception as e:
                    print(f"[A][ERROR] controller/apply_action 실패: {e}")
                    return
...
class RobotBRunner:
    ...
    def __init__(self, robot, controller, ros_bridge: MultiRobotRosBridge, camera_prim_path, task: MultiRobotOilingTask):
        ...
        self.run_state = "WAIT_DOOR_OPEN"  # 덮개는 이미 USD 씬 설정으로 열려있어 마개(blue) lock부터 시작
        self.sub_phase = None
        self.lock_acquirer = None
        self.sequence = None
    ...
    def on_play_reset(self):
        ...
        # 시작하자마자 cap lock을 보지 않고, door가 먼저 열릴 때까지 기다린다.
        ...
        if self.joint6_index is None:
            print("[B-6][WARN] dof_names 안에서 'joint_6'을 찾지 못함")
        elif self.robot.dof_names[self.joint6_index] != "joint_6":
            print(f"[B-6][WARN] joint6_index={self.joint6_index} 가 "
                f"'{self.robot.dof_names[self.joint6_index]}'를 가리킴")

        self._a_done_count_at_reset = self.ros_bridge.robot_a_done_count
        set_prim_visibility(self.task.cap_prim_path, True)

        print(f"[B] RESET -> WAIT_DOOR_OPEN "
            f"(fuel_door {COVER_OPEN_DEG:.0f}도 열림 대기)")

    def _start_wait_blue(self):
        """fuel_door가 열린 뒤 cap(blue) lock 단계로 진입한다."""
        self.lock_acquirer = StableTargetLockAcquirer(
            self.ros_bridge,
            self.camera_prim_path,
            self.task.cap_world_position,
            apply_mouth_offset=False,
        )
        self.wait_steps = 0
        self.ros_bridge.publish_mode_switch("blue")
        self.run_state = "WAIT_BLUE"
        print("\n[B-0] fuel_door open 완료 -> mode_switch=blue, WAIT_BLUE 시작\n")
    ...
    def tick(self, step_count: int):
        ...
        # ---------------- B-0: WAIT_DOOR_OPEN ----------------
        # fuel_door가 USD drive로 먼저 열릴 때까지 대기한다.
        # 문이 충분히 열린 뒤에야 cap(blue) 탐지를 시작한다.
        if self.run_state == "WAIT_DOOR_OPEN":
            self.wait_steps += 1

            door_angle = self.task.current_door_angle_deg()
            door_open_ok = abs(door_angle - COVER_OPEN_DEG) <= DOOR_ANGLE_TOLERANCE_DEG

            if door_open_ok:
                self.door_open_hold_count += 1
            else:
                self.door_open_hold_count = 0

            if step_count % PRINT_EVERY_N_STEPS == 0:
                print(f"[B][WAIT_DOOR_OPEN] door_angle={door_angle:.1f}/"
                    f"{COVER_OPEN_DEG:.1f} hold={self.door_open_hold_count}/"
                    f"{WAIT_DOOR_OPEN_HOLD_STEPS}")

            if self.door_open_hold_count >= WAIT_DOOR_OPEN_HOLD_STEPS:
                self._start_wait_blue()
                return

            if self.wait_steps >= WAIT_DOOR_OPEN_TIMEOUT_STEPS:
                # 데모 안정성용 fallback:
                # 자동 열림 drive만으로 angle 판정이 안 나면 시각적으로 열린 상태로 맞추고 진행한다.
                print(f"\n[B][WAIT_DOOR_OPEN][WARN] timeout. "
                    f"door_angle={door_angle:.1f} -> fuel_door를 {COVER_OPEN_DEG:.1f}도로 강제 설정 후 진행\n")
                self.task.set_door_angle_deg(
                    COVER_OPEN_DEG,
                    label="fuel_door open timeout force",
