...
INSERTION_DIRECTION = -PORT_OUTWARD_NORMAL  # 삽입 방향은 바깥 방향의 정반대 >> 그래서 -
...
# 커버(fuel_door)가 열리는 각도 단계: 0도(닫힘 기준) -> 100도(완전열림).
...
# 100 -> 30 -> 0도(=COVER_START_DEG, 닫힘 기준)로 같이 갱신한다 (set_door_angle_deg).
...
def set_door_prim_visual_angle_deg(
    ...
    # 힌지를 원점으로 옮겼다가 회전하고 되돌리는, 경첩 회전의 표준 공식
    # [힌지가 (5,0,0)에 있을 때]

    # 1단계: 전체를 (-5,0,0) 이동  →  힌지가 원점으로
    # 2단계: 원점 기준 45° 회전
    # 3단계: 전체를 (+5,0,0) 이동  →  힌지가 원래 위치로 복귀
...
def transform_camera_point_to_world(point_camera_ros: np.ndarray, camera_prim_path: str):
    ...
    # 넘파이 배열을 바로 사용할 수 없어 float로 변환 해서 꺼냄.

    # ROS에서 받은 카메라 기준 3D 좌표 
...
# 하드코딩된 기준으로 측정하는 것이므로 문제가 된다면 제거필요
...
# ============================================================
def build_fuel_port_sequence(fuel_port_center: np.ndarray,
                             hold_steps_insert: int = 80) -> WaypointSequence:
    ...
    stages = [
        ...
        FuelStage("04_insert_into_cylinder", insert_target, hold_steps=hold_steps_insert, tolerance=0.05, speed=INSERT_TARGET_SPEED),
...
class MultiRobotRosBridge(Node):
    ...
    def __init__(self):
        ...
        self.start_requested    = False   # 웹UI "주유 시작" 버튼 → Play 트리거
        self.stop_requested     = False   # 웹UI "정지" 버튼 → insert 단계 즉시 종료
        self.fuel_target_liters = 0.0     # 설정한 목표 주유량(L). 0이면 기본 hold_steps 사용
...
class RobotARunner:
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "WAIT_LOCK_HOLE":
            ...
            if mean is not None or self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
                # 목표 주유량 → 삽입 단계 hold_steps 계산 (웹UI와 동기화)
                # 공식: fill_sec = 1 + target_liters * 2000 / 5000  → hold_steps = fill_sec / PHYSICS_DT
                target_l = self.ros_bridge.fuel_target_liters
                if target_l > 0:
                    fill_sec = 1.0 + target_l * 2000.0 / 5000.0
                    hold_steps_insert = max(1, int(fill_sec / PHYSICS_DT))
                else:
                    hold_steps_insert = 80  # 기본값 (~1.3초)

                if mean is not None:
                    self.sequence = build_fuel_port_sequence(mean, hold_steps_insert=hold_steps_insert)
                    self.run_state = "RUN_SEQUENCE"
                    print(f"\n[A] 주유구 위치 확인 완료: center={np.round(mean, 4)} "
                          f"(주유 hold={hold_steps_insert}스텝={hold_steps_insert*PHYSICS_DT:.1f}초) -> 주유 시작\n")
                else:
                    self.sequence = build_fuel_port_sequence(self.task.hole_world_position,
                                                             hold_steps_insert=hold_steps_insert)
                    self.run_state = "RUN_SEQUENCE"
        ...
        if self.run_state == "RUN_SEQUENCE":
            ...
            if stage.name == "04_insert_into_cylinder" and self.ros_bridge.stop_requested:
                ...
                self.sequence.hold_count = stage.hold_steps  # "이미 충분히 유지됨"으로 강제 설정
...
def main():
    ...
    while simulation_app.is_running():
        ...
        # 웹UI "주유 시작" 버튼 → Isaac Sim Play 자동 트리거
        if not is_playing and ros_bridge.start_requested:
            ros_bridge.start_requested = False
            my_world.play()
            print("\n[웹UI] /start_fueling 수신 → 시뮬레이션 Play 시작\n")
