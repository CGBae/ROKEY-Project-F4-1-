...
# 카메라로 어떤 표적(노란/파란/초록)을 찾을지는 detector 노드에 모드 전환 명령을 보내서 정한다.
# detector 구현체는 /color_detector/* 토픽 인터페이스만 맞으면 교체 가능하다
# (색상 contour 기반 multi_color_detector.py 또는 ArUco marker pose 기반 aruco_marker_detector_visual_test.py).
...
# "다 열렸다"고 가정하고 다음 단계(ArUco yellow lock)로 넘어간다.
...
CAP_POSITION_OFFSET = np.array([0.0, 0.0, -0.1], dtype=float)
...
def transform_camera_point_to_world(point_camera_ros: np.ndarray, camera_prim_path: str):
    ...
    detector 노드(multi_color_detector.py / aruco_marker_detector_visual_test.py)가 보내는
...
# ============================================================
# ROS2 bridge: detector 노드(multi_color_detector.py / aruco_marker_detector_visual_test.py)
...
# WAIT_DOOR_OPEN -> WAIT_BLUE -> MOVE_TO_CAP -> GRIP_UNSCREW -> RETURN_HOME_WITH_CAP ->
...
#  yellow lock으로 덮개의 실제 위치를 한 번 확정한다 - 이 값을
...
class RobotBRunner:
    ...
    def __init__(self, robot, controller, ros_bridge: MultiRobotRosBridge, camera_prim_path, task: MultiRobotOilingTask):
        ...
        self.locked_door_center = task.door_world_position.copy()  # WAIT_DOOR_OPEN에서 yellow lock으로 갱신됨
    ...
    def on_play_reset(self):
        ...
        곧바로 카메라에 "노란색(덮개)부터 찾아"라고 명령(mode_switch=yellow)을 보낸다.
        덮개가 USD 씬 설정으로 자동 열리는 동안 로봇은 WAIT_DOOR_OPEN에서 가만히 기다리고,
        다 열리면 이 yellow lock으로 확정한 위치를 CLOSE_COVER의 닫기 기준점으로 쓴다.
        ...
        self.sub_phase = None
        self.lock_acquirer = None  # WAIT_BLUE 진입 시 생성
        ...
        print("[B] RESET -> WAIT_DOOR_OPEN (mode_switch=yellow 발행)")
    ...
    def tick(self, step_count: int):
        ...
        if self.run_state == "WAIT_DOOR_OPEN":
            ...
            if self._door_open_wait_steps < DOOR_OPEN_WAIT_STEPS:
                if step_count % PRINT_EVERY_N_STEPS == 0:
                    print(f"[B][WAIT_DOOR_OPEN] {self._door_open_wait_steps}/{DOOR_OPEN_WAIT_STEPS}스텝 "
                          f"({DOOR_OPEN_FIXED_WAIT_SECONDS:.0f}초) 고정 대기 중 "
                          f"(참고 door_angle={self.task.current_door_angle_deg():.1f}, 신뢰하지 않음)")
                return

            if not self._door_open_announced:
                self._door_open_announced = True
                print(f"\n[B-3] {DOOR_OPEN_FIXED_WAIT_SECONDS:.0f}초 고정 대기 완료 -> "
                      "문이 다 열렸다고 가정하고 yellow lock 시작\n")

            # 고정 대기가 끝났으니, 이제부터 ArUco yellow lock으로 덮개의 실제 위치를 확정한다
            # (CLOSE_COVER에서 하드코딩 상수 대신 이 값을 닫기 기준점으로 쓴다).
            self.wait_steps += 1
            mean = self.door_lock_acquirer.update()
            if mean is not None:
                self.locked_door_center = mean
                print(f"\n[B-3] yellow lock 완료. fuel_door(닫힘 등가) center={np.round(mean, 4)}\n")
            elif self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
                self.locked_door_center = self.task.door_world_position.copy()
                print("\n[B] WAIT_DOOR_OPEN yellow lock timeout -> 하드코딩/USD 기준값으로 다음 단계 진행\n")
            else:
                if step_count % PRINT_EVERY_N_STEPS == 0:
                    print(f"[B][WAIT_DOOR_OPEN] yellow lock 대기 "
                          f"samples={len(self.door_lock_acquirer.samples)}/{CONTROLLER_REQUIRED_LOCK_SAMPLES}")
                return
            ...
            print("[B] WAIT_DOOR_OPEN 완료 -> WAIT_BLUE (mode_switch=blue 발행)")
            return

        # ---------------- B-4: WAIT_BLUE ----------------
        # 문이 다 열린 뒤에만 이 상태에 들어오므로, 파란색(마개) lock에만 집중한다.
        if self.run_state == "WAIT_BLUE":
            ...
            if mean is not None:
                ...
                print(f"\n[B-4] blue lock 완료. fuel_cap center={np.round(self.locked_cap_center, 4)}\n")
                return
            if self.wait_steps >= WAIT_LOCK_TIMEOUT_STEPS:
        ...
        if self.run_state == "RESTORE_CAP":
            ...
            if self.sub_phase == "retreat":
                ...
                if done:
                    ...
                    # ArUco yellow lock으로 확정한 위치(locked_door_center)를 쓴다. marker_to_door_xyz가
