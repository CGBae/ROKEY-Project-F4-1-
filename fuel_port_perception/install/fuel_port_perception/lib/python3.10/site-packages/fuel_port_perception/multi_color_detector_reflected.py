...
class MultiColorDetectorNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("max_depth_m", 1.20)   # 이보다 먼 깊이도 무효 처리
        ...
        self.declare_parameter("blue_low", [90, 80, 40])
    ...
    def mode_switch_callback(self, msg: String) -> None:
        ...
        # 그리고 이전 색에서 남아 있던 lock=True가 다음 색 단계로 넘어가지 않도록 즉시 False를 발행한다.
    ...
    def image_callback(self, rgb_msg: Image, depth_msg: Image) -> None:
        ...
        if detection is not None:
            u, v, area, bbox = detection  # u,v = 화면 픽셀 좌표, area = 면적, bbox = 사각 테두리
            debug_detection = (u, v, bbox)
            edge_hit = self.is_near_edge(u, v, rgb.shape[1], rgb.shape[0])
            # 그 픽셀 위치의 깊이(카메라로부터의 거리)를 읽는다.
    ...
    def detect_color_target(self, rgb: np.ndarray, mode: str) -> Optional[Tuple[int, int, float, Tuple[int, int, int, int]]]:
        """현재 모드(mode) 색깔의 덩어리 중 가장 큰 것을 찾아서 (중심픽셀, 면적, 테두리)를 반환한다."""
    ...
    def get_valid_depth(self, depth_m: np.ndarray, u: int, v: int) -> Optional[float]:
        """픽셀(u,v) 주변 depth_window x depth_window 영역에서 유효한 깊이값들의 중앙값을 구한다.

        한 픽셀만 보면 노이즈 한 번에 흔들리기 쉬우니, 작은 영역의 "중앙값"을 써서 더 안정적으로 만든다.
        """
        h, w = depth_m.shape[:2]
        half = self.depth_window // 2
        # 이미지 경계를 벗어나지 않도록 범위를 잘라준다 (max/min으로 클램핑).
        u0 = max(0, u - half)
        u1 = min(w, u + half + 1)
        v0 = max(0, v - half)
        v1 = min(h, v + half + 1)
        patch = depth_m[v0:v1, u0:u1].astype(np.float32)
        valid = patch[np.isfinite(patch)]  # NaN/inf 같은 이상값 제거
        # 너무 가깝거나(min_depth_m 이하) 너무 먼(max_depth_m 이상) 값도 신뢰하지 않는다.
